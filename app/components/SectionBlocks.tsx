...
// Small, reusable presentational blocks shared by the seven tabbed
// sections' *Tabs.tsx components — same visual language as BookTabs.tsx's
// own bullet/timeline/quote-card treatments, factored out once instead of
// re-implemented seven times.

export function Prose({ text }: { text: string }) {
  const paragraphs = splitParagraphs(text);
  if (paragraphs.length === 0) return <NotWritten label="This" />;
  return (
...
export function BulletList({ items, label = "This" }: { items: string[]; label?: string }) {
  if (items.length === 0) return <NotWritten label={label} />;
  return (
    ...
    <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
      {quotes.map((q, i) => (
        ...
        >
          ...
          <div className="mt-3 text-xs not-italic text-muted">— {q.attribution}</div>
        </blockquote>
      ))}
    </div>
  );
...
export function CriticalTakeBlock({ take }: { take: CriticalTake | undefined }) {
  if (!take || take.points.length === 0) return <NotWritten label="Critical Take" />;
  return (
    <>
      <ul className="space-y-3">
...
export function RoleList({ roles, label = "This" }: { roles: RoleRef[]; label?: string }) {
  if (roles.length === 0) return <NotWritten label={label} />;
  return (
...
export function TextItems({ items, label = "This" }: { items: TextItem[]; label?: string }) {
  if (items.length === 0) return <NotWritten label={label} />;
  return (
    <ul className="space-y-3">
      {items.map((t, i) => (
        <li key={i} className="elevate-sm rounded-lg border-2 border-orange-600/60 bg-surface p-3">
          <div className="text-sm font-semibold">{t.title}</div>
          {t.description && <p className="prose-reading mt-1 text-sm">{t.description}</p>}
        </li>
      ))}
    </ul>
  );
