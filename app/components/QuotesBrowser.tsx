...
// Quotes — the exception, no per-entry tab structure. Flat, filterable
// browsing list (by category, by speaker); tapping a quote expands a small
// non-tabbed card in place. See docs/SECTIONS_SCHEMA.md and New Section
// Research/Section_Detail_Tab_Structures.md §9.
...
export default function QuotesBrowser({ quotes }: { quotes: BrowsableQuote[] }) {
  ...
  if (quotes.length === 0) {
    return <p className="text-sm text-muted">No quotes written yet.</p>;
  }
            ...
            <li key={q.id}>
              ...
              {expanded && (
                <div className="elevate-sm -mt-2 rounded-b-xl border-2 border-t-0 border-orange-600/30 bg-surface2 p-4 text-sm">
                  ...
                  {q.speakerHref && (
                    <Link href={q.speakerHref} className="tap-target mt-3 inline-block text-sm text-jade-400 hover:underline">
